...
namespace FFmpeg.NET
{
    internal static class FFmpegArgumentBuilder
    {
        ...
        private static string Convert(MediaFile inputFile, MediaFile outputFile, ConversionOptions conversionOptions)
        {
            ...
            if (conversionOptions.VideoCodecTune != VideoCodecTune.Default)
                commandBuilder.AppendFormat(" -tune {0} ", conversionOptions.VideoCodecTune);
            ...
            if (conversionOptions.EndDuration != null && conversionOptions.Seek != null)
                commandBuilder.AppendFormat(" -t {0} ", conversionOptions.EndDuration.Value.TotalSeconds - conversionOptions.Seek.Value.TotalSeconds);
            ...
            //Video crf
            if (conversionOptions.VideoCRF != null)
                commandBuilder.AppendFormat(" -crf {0} ", conversionOptions.VideoCRF);
            ...
            if (conversionOptions.AudioBitRate != null)
                commandBuilder.AppendFormat(" -b:a {0}k", conversionOptions.AudioBitRate);
            ...
            if(conversionOptions.AdjustVolumn != null)
            {
                commandBuilder.AppendFormat(" -af volume={0}dB ", conversionOptions.AdjustVolumn);
            }

            if (conversionOptions.AudioQuality != null)
            {
                commandBuilder.AppendFormat(" -aq {0} ", conversionOptions.AudioQuality);
            }
        ...
        private static StringBuilder AppendVideoCodec(StringBuilder commandBuilder, ConversionOptions conversionOptions)
        {
            if (conversionOptions.VideoCodec != VideoCodec.Default)
            {
                ...
                if (codec.Contains("__"))
                    codec = codec.Replace("__", "-");
                commandBuilder.AppendFormat(" -vcodec {0} ", codec);
            }
            return commandBuilder;
        }

        private static StringBuilder AppendAudioCodec(StringBuilder commandBuilder, ConversionOptions conversionOptions)
        {
            if (conversionOptions.AudioCodec != AudioCodec.Default)
            {
                var codec = conversionOptions.AudioCodec.ToString().ToLowerInvariant();
        ...
        private static StringBuilder AppendAudioFormat(StringBuilder commandBuilder, ConversionOptions conversionOptions)
        {
            if (conversionOptions.AudioFormat != AudioFormat.Default)
            {
                var format = conversionOptions.AudioFormat.ToString().ToLowerInvariant();
                if (format.StartsWith("_"))
                    format = format.Replace("_", "");
